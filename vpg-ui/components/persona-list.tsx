...
export default function PersonaList({
  ...
  return (
    <div className="w-80 bg-gray-900 border-r border-gray-800 h-screen flex flex-col fixed left-0 top-0 z-10">
      {/* Header with Logo and Collapse */}
      <div className="p-4 border-b border-gray-800">
        <div className="flex items-center justify-between mb-4">
          <h1 className="text-white font-bold text-lg">vpg.ai</h1>
      ...
      <div className="px-4 mb-4">
        <div className="relative">
          <input
            ...
            className="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-2 pl-10 text-gray-200 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all"
          />
      ...
      {/* This Month Section */}
      <div className="px-4 mb-4">
        <h3 className="text-gray-300 font-semibold text-sm mb-3">This Month</h3>
        <div className="space-y-3">
          {staticAvatars.map((avatar, index) => (
            <div key={index} className="flex items-center gap-3 p-2 rounded-lg hover:bg-gray-800 transition-colors cursor-pointer">
              <AvatarPlaceholder name={avatar.name} size="sm" />
              <div className="flex-1 min-w-0">
                <p className="text-gray-200 text-sm font-medium truncate">{avatar.name}</p>
                <p className="text-gray-400 text-xs truncate">{avatar.description}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="flex-1 px-4 overflow-y-auto">
        <h3 className="text-gray-300 font-semibold text-sm mb-3">Your Personas</h3>
        <div className="space-y-1">
          {filteredItems.map((p) => (
            <button
              key={p.id}
              onClick={() => onSelect(p)}
              className="w-full text-left p-3 rounded-lg transition-all duration-200 text-gray-300 hover:bg-gray-800 hover:text-white"
            >
      ...
      <div className="p-4 border-t border-gray-800">
        <div className="bg-gray-800 rounded-lg p-3">
          ...
          <button className="w-full bg-gradient-to-r from-gray-700 to-gray-600 hover:from-gray-600 hover:to-gray-500 text-gray-200 text-sm font-medium py-2 px-3 rounded-md transition-all duration-200">
            Upgrade to VPG+
          </button>
        </div>
      </div>
